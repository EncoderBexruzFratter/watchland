...
import { Link, useParams, useNavigate } from 'react-router-dom'; // useNavigate ni import qilish
...
import { Videos } from '../'; // Faqat kerakli Videos komponenti import qilinmoqda

const VideoDetail = () => {
  const [videoDetail, setVideoDetail] = useState(null);
  const [relatedVideo, setRelatedVideo] = useState([]);
  const { id } = useParams();
  const navigate = useNavigate(); // useNavigate ni initialize qilish

  useEffect(() => {
    const getData = async () => {
      try {
        const data = await Api_Service.fetching(`videos?part=snippet,statistics&id=${id}`);
        setVideoDetail(data.items[0]);

        const relatedData = await Api_Service.fetching(
